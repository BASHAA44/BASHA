...
tdcli_function({ID ="GetChat",chat_id_=GroupID},function(arg,data) redis:set(basha..'group:name'..GroupID,data.title_) end,nil)
return redis:get(basha..'group:name'..GroupID)
end
function rem_data_group(id_group)
redis:del(basha..'group:add'..id_group) redis:srem(basha..'group:ids',id_group) redis:del(basha..'group:name'..id_group) redis:del(basha..'lock_link'..id_group) redis:del(basha..'lock_id'..id_group) redis:del(basha..'lock_spam'..id_group) redis:del(basha..'lock_webpage'..id_group) redis:del(basha..'lock_markdown'..id_group) redis:del(basha..'lock_flood'..id_group) redis:del(basha..'lock_bots'..id_group) redis:del(basha..'mute_forward'..id_group) redis:del(basha..'mute_contact'..id_group) redis:del(basha..'mute_location'..id_group) redis:del(basha..'mute_document'..id_group) redis:del(basha..'mute_keyboard'..id_group) redis:del(basha..'mute_game'..id_group) redis:del(basha..'mute_inline'..id_group) redis:del(basha..'num_msg_max'..id_group) redis:del(basha..'extimeadd'..id_group) redis:del(basha..'CheckExpire::'..id_group)redis:del(basha..'admins:'..id_group)redis:del(basha..'whitelist:'..id_group)redis:del(basha..'owners:'..id_group)redis:del(basha..'klmamn3'..id_group)redis:del(basha..'lock_edit'..id_group) redis:del(basha..'lock_link'..id_group)redis:del(basha..'lock_tag'..id_group)redis:del(basha..'lock_username'..id_group) redis:del(basha..'lock_pin'..id_group) redis:del(basha..'lock_bots_by_kick'..id_group) redis:del(basha..'mute_gif'..id_group) redis:del(basha..'mute_text'..id_group) redis:del(basha..'mute_photo'..id_group) redis:del(basha..'mute_video'..id_group) redis:del(basha..'mute_audio'..id_group) redis:del(basha..'mute_voice'..id_group) redis:del(basha..'mute_sticker'..id_group) redis:del(basha..'mute_tgservice'..id_group)redis:del(basha..'welcome'..id_group)redis:del(basha..'replay'..id_group) redis:del(basha..'lock_woring'..id_group)
local names = redis:hkeys(basha..'replay:'..id_group)
for i=1, #names do redis:hdel(basha..'replay:'..id_group,names[i]) end
end
...
redis:set(basha..':WITTING:ON',msg.to.id)
redis:setex(basha..":RUN_BOT",5,true)
...
redis:set(basha..'linkgp'..GroupID,linkx)
return linkx
end
...
local var = '' 
if msgs < 80 then var = 'غير متفاعل ✘' elseif msgs < 100 then var = 'ضعيف 🥀' elseif msgs < 150 then var = 'متوسط 🎋' elseif msgs < 500 then var = 'متفاعل 💐' elseif msgs < 1000 then var = 'قوي جدا ⚡️' elseif msgs < 2000 then var = 'قمه التفاعل ✨' elseif msgs < 3000 then var = 'اقوى تفاعل 🔥' end return var end
...
if redis:sismember(basha..':SUDO_BOT:',msg.from.id) then var = true end return var
end
function is_sudo1(user_id)
local var = false
if redis:sismember(basha..':SUDO_BOT:',user_id) then var = true end
if (user_id == SUDO_ID) then var = true end return var
end
function is_monsha(msg)
local var = false
if redis:sismember(basha..':MONSHA_BOT:'..msg.to.id,msg.from.id) then var = true end return var
end
function is_monsha2(user_id,group_id)
local var = false
if redis:sismember(basha..':MONSHA_BOT:'..group_id,user_id) then var = true end return var
end
function is_owner(msg)
local var = false
if redis:sismember(basha..':MONSHA_BOT:'..msg.to.id,msg.from.id) then var = true end
if redis:sismember(basha..':SUDO_BOT:',msg.from.id) then var = true end
if redis:sismember(basha..'owners:'..msg.to.id,msg.from.id)then var = true end
...
if redis:sismember(basha..':MONSHA_BOT:'..msg.to.id,msg.from.id) then var = true end
if redis:sismember(basha..':SUDO_BOT:',msg.from.id) then var = true end
if redis:sismember(basha..'admins:'..msg.to.id,msg.from.id)then var = true end
if redis:sismember(basha..'owners:'..msg.to.id,msg.from.id)then var = true end
if (msg.from.id == SUDO_ID) then var = true end return var
end
...
local var = false
if redis:sismember(basha..':SUDO_BOT:',user_id) then var = true  end
if redis:sismember(basha..':MONSHA_BOT:'..chat_id,user_id) then var = true end
if redis:sismember(basha..'owners:'..chat_id,user_id)then var = true end
if (user_id == SUDO_ID) then var = true end return var
end
...
if redis:sismember(basha..':MONSHA_BOT:'..chat_id,user_id) then var = true end
if redis:sismember(basha..':SUDO_BOT:',user_id) then var = true end
if redis:sismember(basha..'admins:'..chat_id,user_id)then var = true end
if redis:sismember(basha..'owners:'..chat_id,user_id)then var = true end
if (user_id == SUDO_ID) then var = true end return var 
end
function is_whitelist(user_id, chat_id)
local var = false
if redis:sismember(basha..'whitelist:'..chat_id,user_id)then var = true end return var
end
function is_banned(user_id, chat_id)
local var = false
if redis:sismember(basha..'banned:'..chat_id,user_id)then var = true end return var
end
function is_silent_user(user_id, chat_id)
local var = false
if redis:sismember(basha..'is_silent_users:'..chat_id,user_id)then var = true end return var
end
function is_gbanned(user_id)
local var = false
if redis:sismember(basha..'gban_users',user_id)then var = true end return var 
end
function is_filter(GroupID, text)
local var = false
local list =redis:smembers(basha..'klmamn3'..GroupID)
if #list ~= 0 then
for i = 1, #list do
...
local list_plug = redis:smembers(basha..':PLUGINS_FILE:')
...
redis:sadd(basha..':PLUGINS_FILE:',name) reload_plugins()
...
redis:srem(basha..':PLUGINS_FILE:',name)
reload_plugins() 
return '🗂*¦* تم تعطيل الملف ✓\n➠ ['..name..']' 
end 
function banned_list(chat_id)
local list = redis:smembers(basha..'banned:'..chat_id)
...
local list = redis:smembers(basha..'is_silent_users:'..chat_id)
...
local list = redis:smembers(basha..'klmamn3'..msg.to.id)
...
if redis:sismember(basha..'klmamn3'..msg.to.id,word) then return  "📝*¦* الكلمه *"..word.."* هي بالتأكيد من قائمه المنع✓️" end
redis:sadd(basha..'klmamn3'..msg.to.id,word) return  "📝*¦* الكلمه *"..word.."* تمت اضافتها الى قائمه المنع ✓️"
end
function unfilter_word(msg, word)
if redis:sismember(basha..'klmamn3'..msg.to.id,word) then redis:srem(basha..'klmamn3'..msg.to.id,word) return  "📝*¦* الكلمه *"..word.."* تم السماح بها ✓️" else return  "📝*¦* الكلمه *"..word.."* هي بالتأكيد مسموح بها✓️" end
end
function modlist(msg)
local list = redis:smembers(basha..'admins:'..msg.to.id)
...
message ='*----------------------------------\n\n📋¦ قائمه المدراء :*\n\n'
local list = redis:smembers(basha..'owners:'..msg.to.id)
...
local list = redis:smembers(basha..'whitelist:'..msg.to.id)
...
message =''
for k,v2 in pairs(redis:smembers(basha..'group:ids')) do 
adminsw = ''
for k,v in pairs(redis:smembers(basha..'admins:'..v2)) do
local info = redis:hgetall(basha..'username:'..v)
if info and info.username then
adminsw = adminsw.."redis:hset('"..basha.."username:"..v.."','username',[["..info.username.."]]) redis:sadd('"..basha.."admins:"..v2.."','"..v.."')"
end end MONSHA_BOT = ''
for k,v in pairs(redis:smembers(basha..':MONSHA_BOT:'..v2)) do
local info = redis:hgetall(basha..'username:'..v)
if info and info.username then
MONSHA_BOT = MONSHA_BOT.."redis:hset('"..basha.."username:"..v.."','username',[["..info.username.."]]) redis:sadd('"..basha..":MONSHA_BOT:"..v2.."','"..v.."')"
end end owners = ''
for k,v in pairs(redis:smembers(basha..'owners:'..v2)) do
local info = redis:hgetall(basha..'username:'..v)
if info and info.username then
owners = owners.."redis:hset('"..basha.."username:"..v.."','username',[["..info.username.."]]) redis:sadd('"..basha.."owners:"..v2.."','"..v.."')"
end end
message = message.."redis:set('"..basha.."group:name"..v2.."',[["..redis:get(basha..'group:name'..v2).."]]) redis:set('"..basha.."group:add"..v2.."',true) redis:sadd('"..basha.."group:ids','"..v2.."') redis:set('"..basha.."lock_link"..v2.."',true) redis:set('"..basha.."lock_spam"..v2.."',true) redis:set('"..basha.."lock_flood"..v2.."',true) redis:set('"..basha.."lock_bots"..v2.."',true) redis:set('"..basha.."mute_forward"..v2.."',true) redis:set('"..basha.."mute_contact"..v2.."',true) redis:set('"..basha.."num_msg_max"..v2.."',5)"
...
if redis:ttl(basha..'ExpireDate:'..msg.to.id) == -1 then return else
local d = math.floor(redis:ttl(basha..'ExpireDate:'..msg.to.id) / 86400) + 1
...
redis:hset(basha..'username:'..data.id_,'username',user_name)
redis:sadd(basha..'admins:'..msg.to.id,data.id_) end
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
redis:hset(basha..'username:'..arg.user_id, 'username', user_name)
redis:sadd(basha..':MONSHA_BOT:'..msg.to.id,arg.user_id)
...
redis:set(basha..'group:add'..msg.to.id,true) redis:sadd(basha..'group:ids',msg.to.id) redis:set(basha..'lock_link'..msg.to.id,true)  redis:set(basha..'lock_id'..msg.to.id,true) redis:set(basha..'lock_spam'..msg.to.id,true) redis:set(basha..'lock_webpage'..msg.to.id,true) redis:set(basha..'lock_markdown'..msg.to.id,true) redis:set(basha..'lock_flood'..msg.to.id,true) redis:set(basha..'lock_bots'..msg.to.id,true) redis:set(basha..'mute_forward'..msg.to.id,true) redis:set(basha..'mute_contact'..msg.to.id,true) redis:set(basha..'mute_location'..msg.to.id,true) redis:set(basha..'mute_document'..msg.to.id,true) redis:set(basha..'mute_keyboard'..msg.to.id,true) redis:set(basha..'mute_game'..msg.to.id,true) redis:set(basha..'mute_inline'..msg.to.id,true) redis:set(basha..'lock_username'..msg.to.id,true) redis:set(basha..'num_msg_max'..msg.to.id,5) redis:sadd(basha..'mtwr_count'..msg.from.id,msg.to.id)
redis:set(basha..'replay'..msg.to.id,true) 
if redis:get(basha..'lock_service') then sendMsg(msg.to.id,msg.id_,'📮*¦  تـم تـفـعـيـل الـمـجـمـوعـه ✓️ \n👨🏽‍🔧¦¦* وتم رفع جمـيع آلآدمـنيهہ‏‏‏ آلگروب بآلبوت \n✓','md')
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
redis:hset(basha..'username:'..data.id_,'username',user_name)
redis:sadd(basha..'admins:'..msg.to.id,data.id_)
end
tdcli_function ({ID = "GetUser",user_id_ = v.user_id_}, config_mods, {user_id=v.user_id_})
if data.members_[k].status_.ID == "ChatMemberStatusCreator" then
local function config_owner(arg, data)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
redis:hset(basha..'username:'..arg.user_id, 'username', user_name)
redis:sadd(basha..':MONSHA_BOT:'..msg.to.id,arg.user_id) end
tdcli_function ({ID = "GetUser",user_id_ = v.user_id_}, config_owner, {user_id=v.user_id_})
end end  end 
group_set(msg,numus)
 end end ,nil) end
function modadd(msg,num)
if not is_sudo(msg) and not redis:get(basha..'lock_service') then return '🚸¦ أنـت لـسـت الـمـطـور ⚙️' end
if msg.to.type ~= "channel" then return '🚸¦ لا يمكنك تفعيل البوت في المجوعات العاديه / البوت يدعم فقط المجموعات الخارقه ⚙️' end
if redis:get(basha..'group:add'..msg.to.id) then  return '🎗*¦* المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
return sendMsg(msg.to.id,0,'🚸*¦* لآ يمـگنني تفعيل آلبوت في آلمـجمـوعهہ‏ يجب آن يگون آگثر مـن *【'..redis:get(basha..':addnumberusers')..'】* عضـو 👤','md')
 else return set_groupadmins(msg,data.member_count_) end 
end,{num=num}) end 
function botrem(msg)
redis:del(basha..'group:add'..msg.to.id) redis:srem(basha..'group:ids',msg.to.id) redis:del(basha..'group:name'..msg.to.id) redis:del(basha..'lock_link'..msg.to.id) redis:del(basha..'lock_id'..msg.to.id) redis:del(basha..'lock_spam'..msg.to.id) redis:del(basha..'lock_webpage'..msg.to.id) redis:del(basha..'lock_markdown'..msg.to.id) redis:del(basha..'lock_flood'..msg.to.id) redis:del(basha..'lock_bots'..msg.to.id) redis:del(basha..'mute_forward'..msg.to.id) redis:del(basha..'mute_contact'..msg.to.id) redis:del(basha..'mute_location'..msg.to.id) redis:del(basha..'mute_document'..msg.to.id) redis:del(basha..'mute_keyboard'..msg.to.id) redis:del(basha..'mute_game'..msg.to.id) redis:del(basha..'mute_inline'..msg.to.id) redis:del(basha..'num_msg_max'..msg.to.id) redis:del(basha..'extimeadd'..msg.to.id) redis:del(basha..'CheckExpire::'..msg.to.id) redis:del(basha..'admins:'..msg.to.id) redis:del(basha..'whitelist:'..msg.to.id) redis:del(basha..'owners:'..msg.to.id)redis:del(basha..'klmamn3'..msg.to.id) redis:del(basha..'lock_edit'..msg.to.id) redis:del(basha..'lock_link'..msg.to.id) redis:del(basha..'lock_tag'..msg.to.id) redis:del(basha..'lock_username'..msg.to.id) redis:del(basha..'lock_pin'..msg.to.id) redis:del(basha..'lock_bots_by_kick'..msg.to.id) redis:del(basha..'mute_gif'..msg.to.id) redis:del(basha..'mute_text'..msg.to.id) redis:del(basha..'mute_photo'..msg.to.id) redis:del(basha..'mute_video'..msg.to.id) redis:del(basha..'mute_audio'..msg.to.id) redis:del(basha..'mute_voice'..msg.to.id) redis:del(basha..'mute_sticker'..msg.to.id) redis:del(basha..'mute_tgservice'..msg.to.id) redis:del(basha..'welcome'..msg.to.id) redis:del(basha..'replay'..msg.to.id) redis:del(basha..'lock_woring'..msg.to.id)
local names = redis:hkeys(basha..'replay:'..msg.to.id)
for i=1, #names do redis:hdel(basha..'replay:'..msg.to.id,names[i]) end
...
if not redis:get(basha..'group:add'..msg.to.id) then return '🛠*¦* المجموعه بالتأكيد ✓️ تم تعطيلها' end  
redis:del(basha..'group:add'..msg.to.id) redis:srem(basha..'group:ids',msg.to.id) redis:del(basha..'group:name'..msg.to.id) redis:del(basha..'lock_link'..msg.to.id) redis:del(basha..'lock_id'..msg.to.id) redis:del(basha..'lock_spam'..msg.to.id) redis:del(basha..'lock_webpage'..msg.to.id) redis:del(basha..'lock_markdown'..msg.to.id) redis:del(basha..'lock_flood'..msg.to.id) redis:del(basha..'lock_bots'..msg.to.id) redis:del(basha..'mute_forward'..msg.to.id) redis:del(basha..'mute_contact'..msg.to.id) redis:del(basha..'mute_location'..msg.to.id) redis:del(basha..'mute_document'..msg.to.id) redis:del(basha..'mute_keyboard'..msg.to.id) redis:del(basha..'mute_game'..msg.to.id) redis:del(basha..'mute_inline'..msg.to.id) redis:del(basha..'num_msg_max'..msg.to.id) redis:del(basha..'admins:'..msg.to.id)redis:del(basha..'whitelist:'..msg.to.id)redis:del(basha..'owners:'..msg.to.id)redis:del(basha..'klmamn3'..msg.to.id) redis:del(basha..'lock_edit'..msg.to.id) redis:del(basha..'lock_link'..msg.to.id)redis:del(basha..'lock_tag'..msg.to.id)redis:del(basha..'lock_username'..msg.to.id) redis:del(basha..'lock_pin'..msg.to.id) redis:del(basha..'lock_bots_by_kick'..msg.to.id) redis:del(basha..'mute_gif'..msg.to.id) redis:del(basha..'mute_text'..msg.to.id) redis:del(basha..'mute_photo'..msg.to.id) redis:del(basha..'mute_video'..msg.to.id) redis:del(basha..'mute_audio'..msg.to.id) redis:del(basha..'mute_voice'..msg.to.id) redis:del(basha..'mute_sticker'..msg.to.id) redis:del(basha..'mute_tgservice'..msg.to.id) redis:del(basha..'welcome'..msg.to.id) redis:del(basha..'replay'..msg.to.id) redis:del(basha..'lock_woring'..msg.to.id) redis:setex(basha..'extimeadd'..msg.to.id, 300 , true)
local names = redis:hkeys(basha..'replay:'..msg.to.id) for i=1, #names do redis:hdel(basha..'replay:'..msg.to.id,names[i]) end
...
redis:hset(basha..'username:'..data.id_, 'username', user_name)
redis:sadd(basha..':tqeed:'..arg.chat_id,data.id_)
...
redis:srem(basha..':tqeed:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(basha..':MONSHA_BOT:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد منشىء \n✓️',"md") end
redis:hset(basha..'username:'..data.id_, 'username', user_name)
redis:sadd(basha..':MONSHA_BOT:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(basha..':MONSHA_BOT:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس منشىء \n✓️', "md") end
redis:srem(basha..':MONSHA_BOT:'..arg.chat_id,data.id_)
...
local function setwhitelist_cb(arg, data)
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(basha..'whitelist:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد من عضو مميز \n✓️',"md") end
redis:hset(basha..'username:'..data.id_, 'username', user_name)
redis:sadd(basha..'whitelist:'..arg.chat_id,data.id_)
...
local function remwhitelist_cb(arg, data)
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(basha..'whitelist:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس من عضو مميز \n✓️',"md") end
redis:srem(basha..'whitelist:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(basha..'owners:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد مدير \n✓️',"md") end
redis:hset(basha..'username:'..data.id_, 'username', user_name)
redis:sadd(basha..'owners:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(basha..'admins:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ادمن \n✓️',"md") end
redis:hset(basha..'username:'..data.id_, 'username', user_name)
redis:sadd(basha..'admins:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(basha..'owners:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس مدير \n✓️',"md") end
redis:srem(basha..'owners:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(basha..'admins:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس ادمن \n✓️',"md") end
redis:srem(basha..'admins:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
msgs = tonumber(redis:get(basha..'msgs:'..data.id_..':'..arg.chat_id) or 1)
...
if redis:sismember(basha..'banned:'..arg.chat_id,data.id_) then
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد حظره \n✓️',"md")
end
redis:hset(basha..'username:'..data.id_, 'username', user_name)
redis:sadd(basha..'banned:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(basha..'banned:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد الغاء حظره \n✓️', "md") end
redis:srem(basha..'banned:'..arg.chat_id,data.id_)
...
if redis:sismember(basha..'is_silent_users:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id, '👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد كتمه \n✓️', "md") end
redis:hset(basha..'username:'..data.id_, 'username', user_name)
redis:sadd(basha..'is_silent_users:'..arg.chat_id,data.id_)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم كتمه \n✓️',"md")
end
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(basha..'is_silent_users:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتاكيد الغاء كتمه \n✓️', "md") end
redis:srem(basha..'is_silent_users:'..arg.chat_id,data.id_)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم الغاء كتمه \n✓️',"md")
end
...
redis:hset(basha..'username:'..data.id_, 'username', user_name)
redis:sadd(basha..'gban_users',data.id_)
kick_user(data.id_, arg.chat_id)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم حظره عام  \n✓️', "md")
end
...
redis:srem(basha..'gban_users',data.id_)
channel_unblock(arg.chat_id, arg.user_id)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم الغاء حظره العام \n✓️', "md")
end
...
redis:hset(basha..'username:'..data.id_, 'username', user_name)
redis:sadd(basha..':SUDO_BOT:',data.id_)
...
redis:srem(basha..':SUDO_BOT:',data.id_)
...
if redis:sismember(basha..'banned:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد حظره \n✓️', "md") end
redis:hset(basha..'username:'..data.id_, 'username', user_name)
redis:sadd(basha..'banned:'..arg.chat_id,data.id_)
kick_user(data.id_, arg.chat_id)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم حظره \n✓️', "md")
...
if redis:get(basha..'welcome'..msg.to.id) then welcomex = "{ مفعل }" else welcomex = "{ معطل }" end
if redis:get(basha..'lock_id'..msg.to.id) then lock_idx = "{ مفعل }" else lock_idx = "{ معطل }" end
if redis:get(basha..'replay'..msg.to.id)  then replayx  = "{ مفعل }" else replayx  = "{ معطل }" end
if redis:get(basha..'lock_woring'..msg.to.id) then lock_woringx = "{ مفعل }" else lock_woringx  = "{ معطل }" end
list_settings = "*👮🏾¦*` اعدادات المجموعه :` \n\n📝¦ التعديل » "..(redis:get(basha..'lock_edit'..msg.to.id) or 'false').."\n🔗¦ الروابط » "..(redis:get(basha..'lock_link'..msg.to.id) or 'false').."\n#️⃣¦ التاك » "..(redis:get(basha..'lock_tag'..msg.to.id) or 'false').."\n©¦ المعرفات » "..(redis:get(basha..'lock_username'..msg.to.id) or 'false').."\n\n💱¦ التكرار » "..(redis:get(basha..'lock_flood'..msg.to.id) or 'false').."\n📑¦ الكلايش » "..(redis:get(basha..'lock_spam'..msg.to.id) or 'false').."\n🌐¦ الويب » "..(redis:get(basha..'lock_webpage'..msg.to.id) or 'false').."\n⚜️¦ الماركدوان » "..(redis:get(basha..'lock_markdown'..msg.to.id) or 'false').."\n🏌🏻¦ البوتات بالطرد » "..(redis:get(basha..'lock_bots_by_kick'..msg.to.id) or 'false').."\n🤖¦ البوتات » "..(redis:get(basha..'lock_bots'..msg.to.id) or 'false').."\n➕¦ عدد التكرار » "..(redis:get(basha..'num_msg_max'..msg.to.id) or 'false').."\n\n🎬¦` اعدادات الوسائط :`\n\n*¦* المتحركه » "..(redis:get(basha..'mute_gif'..msg.to.id) or 'false').."\n💭¦ الدردشه » "..(redis:get(basha..'mute_text'..msg.to.id) or 'false').."\n◽️¦ الانلاين » "..(redis:get(basha..'mute_inline'..msg.to.id) or 'false').."\n🎮¦ الالعاب » "..(redis:get(basha..'mute_game'..msg.to.id) or 'false').."\n🏞¦ الصور » "..(redis:get(basha..'mute_photo'..msg.to.id) or 'false').."\n🎥¦ الفيديو » "..(redis:get(basha..'mute_video'..msg.to.id) or 'false').."\n🎙¦ البصمات » "..(redis:get(basha..'mute_audio'..msg.to.id) or 'false').."\n\n🔉¦ الصوت » "..(redis:get(basha..'mute_voice'..msg.to.id) or 'false').."\n🎎¦ الملصقات » "..(redis:get(basha..'mute_sticker'..msg.to.id) or 'false').."\n📞¦ الجهات » "..(redis:get(basha..'mute_contact'..msg.to.id) or 'false').."\n💱¦ التوجيه » "..(redis:get(basha..'mute_forward'..msg.to.id) or 'false').."\n🌐¦ الموقع » "..(redis:get(basha..'mute_location'..msg.to.id) or 'false').."\n🗂¦ الملفات » "..(redis:get(basha..'mute_document'..msg.to.id) or 'false').."\n🔅¦ الاشعارات » "..(redis:get(basha..'mute_tgservice'..msg.to.id) or 'false').."\n🔒¦ الكيبورد » "..(redis:get(basha..'mute_keyboard'..msg.to.id) or 'false').."\n\n*⚒¦*` اعدادات اخرى : `\n*🙋🏼‍♂️¦* الترحيب » "..welcomex.."\n*💬¦*  الردود » "..replayx.."\n*🚸¦*  التحذير » "..lock_woringx.."\n*🎫¦* الايدي » "..lock_idx
