...
#!/usr/bin/env bash
cd $(cd $(dirname $0); pwd)
if ! screen -list | grep -q "basha"; then
screen -S basha lua ./inc/BOT.lua
fi
screen -X -S basha kill
screen -S basha lua ./inc/BOT.lua
]])
file:close()
...
]]..'\27[0;31m'..[[¦ Dev : @mokhtar77]]..'\27[m\27[0;32m\n\n\027[0;32m'   
..'\27[0;31m'..[[¦ Dev : @vpsba]]..'\27[m\27[0;32m\n\n\027[0;32m'   
...
_info = load_config()
basha = _info.TOKEN:match("(%d+)")
our_id = tonumber(basha)
bot_username = redis:get(basha..":username")
...
print('\n\27[0;32mPlugins is : '..#list_plug..' Are Active.\n\n\27[0;31mThe SOURCE RUN IS BASHA v'..redis:get(basha..':VERSION')..'...\n\27[m')
end
load_plugins()
...
return false end 
if not (tostring(msg.chat_id_):match('^-100') or tostring(msg.chat_id_):match('-')) and redis:get(basha..':mute_pv:'..msg.sender_user_id_) then
...
if redis:get(basha..'mute_text'..msg.chat_id_) then --قفل الدردشه
