 BOT_NAME = redis:get(basha..'bot:name')
do 
...
redis:setex(basha..'namebot:witting'..msg.from.id,300,true)
...
local keyboard = {
 ...
 {"تحديث السورس ™","تفعيل البوت خدمي","السيرفر"},
 {"السورس","ضع كليشه المطور","تعطيل البوت خدمي"}
 {"نسخه احتياطيه للمجموعات","المجموعات","تيست"}
 {"ضعشرط التفعيل","شرط التفعيل","قائمه المجموعات"}
 {"الغاء الامر ✖️"}}
return send_inline_key(msg.from.id,text,keyboard)
else
 redis:sadd(basha..'users',msg.from.id)
...
if (msg.to.type == "pv") and not we_sudo(msg) and not redis:get(basha..'lock_twasel') and msg.from.id ~= our_id then -- ارسال رساله للاعضاء الي يدخلون خاص
...
local inline = {{{text="قنآهہ‏‏ آلسـورس : الباشا 🍃",url="t.me/VPSBA"}}}
return send_inline_key(msg.from.id,'  [قناة السورس : الباشا](t.me/VPSBA)',nil,inline)
end
if (r== "الاحصائيات 📈" or r=="الاحصائيات") and is_sudo(msg) then
local group = redis:smembers(basha..'group:ids')
...
if not we_sudo(msg) then return "☔️هذا الاوامر للمطور الاساسي فقط 🌑" end
redis:setex(basha..'addrd_all:'..msg.to.id..msg.from.id,300 , true)
redis:del(basha..'allreplay:'..msg.to.id..msg.from.id)
...
if redis:get(basha..'namebot:witting'..msg.from.id) then --- استقبال اسم البوت 
redis:del(basha..'namebot:witting'..msg.from.id)
redis:set(basha..'bot:name',msg.text)
reload_plugins() 
return "📭¦ تم تغير اسم البوت  ✋🏿\n🗯¦ الان اسمه `"..msg.text.."` \n✓"
end
if redis:get(basha..'addrd_all:'..msg.to.id..msg.from.id) then -- استقبال الرد لكل المجموعات
if not redis:get(basha..'allreplay:'..msg.to.id..msg.from.id) then-- استقبال كلمه الرد لكل المجموعات
...
redis:setex(basha..'allreplay:'..msg.to.id..msg.from.id,300,msg.text)
return "📜¦ جيد , يمكنك الان ارسال جوا ب الردالعام \n🔛¦ [[ نص,صوره,فيديو,متحركه,بصمه,اغنيه ]] ✓\n-" 
end
if redis:get(basha..'allreplay:'..msg.to.id..msg.from.id) then -- استقبال جواب الرد لكل المجموعات
redis:hset(basha..'replay:all', redis:get(basha.."allreplay:"..msg.to.id..msg.from.id),msg.text)
redis:del(basha..'addrd_all:'..msg.to.id..msg.from.id)
return '(['..redis:get(basha..'allreplay:'..msg.to.id..msg.from.id)..'])\n  ✓ تم اضافت الرد لكل المجموعات 🚀 '
end end
-------------------------------------------------------------
if redis:get(basha..'addrd:'..msg.to.id..msg.from.id) then -- استقبال الرد للمجموعه فقط
if not redis:get(basha..'replay1'..msg.to.id..msg.from.id) then  -- كلمه الرد
redis:hdel(basha..'replay:'..msg.to.id,msg.text)
redis:hdel(basha..'replay_photo:group:'..msg.to.id,msg.text)
redis:hdel(basha..'replay_voice:group:'..msg.to.id,msg.text)
redis:hdel(basha..'replay_animation:group:'..msg.to.id,msg.text)
redis:hdel(basha..'replay_audio:group:'..msg.to.id,msg.text)
redis:hdel(basha..'replay_sticker:group:'..msg.to.id,msg.text)
redis:hdel(basha..'replay_video:group:'..msg.to.id,msg.text)
redis:setex(basha..'replay1'..msg.to.id..msg.from.id,300,msg.text)
return "📜¦ جيد , يمكنك الان ارسال جواب الرد \n🔛¦ [[ نص,صوره,فيديو,متحركه,بصمه,اغنيه ]] ✓\n-" 
end
if redis:get(basha..'replay1'..msg.to.id..msg.from.id) then -- جواب الرد
redis:hset(basha..'replay:'..msg.to.id, redis:get(basha.."replay1"..msg.to.id..msg.from.id),msg.text)
redis:del(basha..'addrd:'..msg.to.id..msg.from.id)
return '(['..redis:get(basha..'replay1'..msg.to.id..msg.from.id)..'])\n  ✓ تم اضافت الرد 🚀 \n-'
end end
if redis:get(basha..'delrd:'..msg.from.id) then
redis:del(basha..'delrd:'..msg.from.id)
local names = redis:hget(basha..'replay:'..msg.to.id,msg.text)
local photo =redis:hget(basha..'replay_photo:group:'..msg.to.id,msg.text)
local voice = redis:hget(basha..'replay_voice:group:'..msg.to.id,msg.text)
local animation = redis:hget(basha..'replay_animation:group:'..msg.to.id,msg.text)
local audio = redis:hget(basha..'replay_audio:group:'..msg.to.id,msg.text)
local sticker = redis:hget(basha..'replay_sticker:group:'..msg.to.id,msg.text)
local video = redis:hget(basha..'replay_video:group:'..msg.to.id,msg.text)
...
if redis:get(basha..'delrdall:'..msg.from.id) then
redis:del(basha..'delrdall:'..msg.from.id)
...
local photo =redis:hkeys(basha..'replay_photo:group:'..msg.to.id)
local voice = redis:hkeys(basha..'replay_voice:group:'..msg.to.id)
local animation = redis:hkeys(basha..'replay_animation:group:'..msg.to.id)
local audio = redis:hkeys(basha..'replay_audio:group:'..msg.to.id)
local sticker = redis:hkeys(basha..'replay_sticker:group:'..msg.to.id)
local video = redis:hkeys(basha..'replay_video:group:'..msg.to.id)
...
for i=1, #photo do redis:hdel(basha..'replay_photo:group:'..msg.to.id,photo[i]) end
for i=1, #voice do redis:hdel(basha..'replay_voice:group:'..msg.to.id,voice[i]) end
for i=1, #animation do redis:hdel(basha..'replay_animation:group:'..msg.to.id,animation[i]) end
for i=1, #audio do redis:hdel(basha..'replay_audio:group:'..msg.to.id,audio[i]) end
for i=1, #sticker do redis:hdel(basha..'replay_sticker:group:'..msg.to.id,sticker[i]) end
for i=1, #video do redis:hdel(basha..'replay_video:group:'..msg.to.id,video[i]) end
...
local animation = redis:hkeys(basha..'replay_animation:group:')
...
for i=1, #names do redis:hdel(basha..'replay:all',names[i]) end
for i=1, #photo do redis:hdel(basha..'replay_photo:group:',photo[i]) end
for i=1, #voice do redis:hdel(basha..'replay_voice:group:',voice[i]) end
for i=1, #animation do redis:hdel(basha..'replay_animation:group:',animation[i]) end
for i=1, #audio do redis:hdel(basha..'replay_audio:group:',audio[i]) end
for i=1, #sticker do redis:hdel(basha..'replay_sticker:group:',sticker[i]) end
for i=1, #video do redis:hdel(basha..'replay_video:group:',video[i]) end
return "✓ تم مسح كل الردود العامه🚀"
end
if r== 'مسح رد عام' then
if not is_sudo(msg) then return"♨️ للمطورين فقط ! 💯" end
redis:set(basha..'delrdall:'..msg.from.id,true) return "📭¦ حسننا عزيزي  ✋🏿\n🗯¦ الان ارسل الرد لمسحها من  المجموعات 🍃"
end
if r== 'مسح رد' then
if not is_owner(msg) then return"♨️ للمدراء فقط ! 💯" end redis:set(basha..'delrd:'..msg.from.id,true)
...
if not redis:get(basha..'group:add'..msg.to.id) then return end
if r=="اضف رد" then
if not is_owner(msg) then return"♨️ للمدراء فقط ! 💯" end
redis:setex(basha..'addrd:'..msg.to.id..msg.from.id,300,true) redis:del(basha..'replay1'..msg.to.id..msg.from.id)
...
if msg.text and redis:get(basha..'replay'..msg.to.id) and msg.to.type == "channel" then
-- by @basha
...
if redis:get(basha..":msg_id"..msg.to.id) then
if msg.reply_id then  msgx_id = msg.reply_id  else msgx_id = false end else msgx_id = msg.id_  end
if msgx_id then
if redis:hget(basha..'replay:all',r) then return sendMsg(msg.to.id,msgx_id,"["..redis:hget(basha..'replay:all',r).."]",'md')
elseif redis:hget(basha..'replay_photo:group:',r) then return sendPhoto(msg.to.id,msgx_id,0,1,nil,redis:hget(basha..'replay_photo:group:',r))  
elseif redis:hget(basha..'replay_voice:group:',r) then return sendVoice(msg.to.id,msgx_id,0,1,nil,redis:hget(basha..'replay_voice:group:',r))
elseif redis:hget(basha..'replay_animation:group:',r) then return sendAnimation(msg.to.id,msgx_id,0,1, nil,redis:hget(basha..'replay_animation:group:',r))  
elseif redis:hget(basha..'replay_audio:group:',r) then return sendAudio(msg.to.id,msgx_id,0,1,nil,redis:hget(basha..'replay_audio:group:',r))  
elseif redis:hget(basha..'replay_sticker:group:',r) then return sendSticker(msg.to.id,msgx_id,0,1,nil,redis:hget(basha..'replay_sticker:group:',r))  
elseif redis:hget(basha..'replay_video:group:',r) then return sendVideo(msg.to.id,msgx_id,0,1,nil,redis:hget(basha..'replay_video:group:',r))
--==============================
elseif redis:hget(basha..'replay_photo:group:'..msg.to.id,r) then return sendPhoto(msg.to.id,msgx_id,0,1,nil,redis:hget(basha..'replay_photo:group:'..msg.to.id, r))  
elseif redis:hget(basha..'replay_voice:group:'..msg.to.id,r) then return sendVoice(msg.to.id,msgx_id,0,1,nil,redis:hget(basha..'replay_voice:group:'..msg.to.id, r))
elseif redis:hget(basha..'replay_animation:group:'..msg.to.id,r) then return sendAnimation(msg.to.id,msgx_id,0,1, nil,redis:hget(basha..'replay_animation:group:'..msg.to.id, r))  
elseif redis:hget(basha..'replay_audio:group:'..msg.to.id,r) then return sendAudio(msg.to.id,msgx_id,0,1,nil,redis:hget(basha..'replay_audio:group:'..msg.to.id, r))  
elseif redis:hget(basha..'replay_sticker:group:'..msg.to.id,r) then return sendSticker(msg.to.id,msgx_id,0,1,nil,redis:hget(basha..'replay_sticker:group:'..msg.to.id, r))  
elseif redis:hget(basha..'replay_video:group:'..msg.to.id,r) then return sendVideo(msg.to.id,msgx_id,0,1,nil, redis:hget(basha..'replay_video:group:'..msg.to.id, r))
elseif redis:hget(basha..'replay:'..msg.to.id,r) then return sendMsg(msg.to.id,msgx_id,"["..redis:hget(basha..'replay:'..msg.to.id, r).."]",'md') 
end else
if redis:hget(basha..'replay:all',r) or redis:hget(basha..'replay_photo:group:'..msg.to.id,r) or redis:hget(basha..'replay_voice:group:'..msg.to.id,r) or redis:hget(basha..'replay_animation:group:'..msg.to.id,r) or redis:hget(basha..'replay_audio:group:'..msg.to.id,r) or  redis:hget(basha..'replay_sticker:group:'..msg.to.id,r) or redis:hget(basha..'replay_video:group:'..msg.to.id,r) or redis:hget(basha..'replay:'..msg.to.id,r) then
...
elseif r== "شكرا" or r== "ثكرا" then return  "{ وملحا وطماطه وخبزة اويلااااه ناقصهة وحدة صاكه تاكل وياي }"
...
elseif r== "مختار"  then return  "ابو اسوله الغالي رحتله فدوة 😍❤️️"
...
elseif r== "تعال خاص" or r== "خاصك" or r=="شوف الخاص" or r=="خاصج" then return  "ها شسون 😉"
...
elseif r== "😳" or r== "😳😳" or r== "😳😳😳" then return  "مبين جديد ع الشغله  😳😹🕷"
...
elseif r== "اشكرك" then return  "تدلل/ي حبي ❤"
...
if is_sudo(msg) then  return   "انته تأمرني باشا 😇 "
elseif is_owner(msg) then return   "تدلل مديرنه   😌"
elseif is_mod(msg) then return   "يعني فوك مامصعدة ادمن يتأمر ابوحلك  😏"
else return   "خاب كوووووم 😏"
elseif r== "بوت" or r== "bot" or r=="بووت" then
if is_sudo(msg) then  return   "مطوري العشق يأمرني امر  😇 "
elseif is_owner(msg) then return   "عيوني المدير شرايد    😌"
elseif is_mod(msg) then return   "يعني ع اساس متعرف اسمي احترم شويه وكول باشا   😏"
else return   "خاب كوووووم 😏"
elseif r== "💋" or r== "😘" or r=="😚" then
if is_sudo(msg) then  return   "💋مممح عسل "
elseif is_owner(msg) then return   "💋"
elseif is_mod(msg) then return   "💦طاحظك تضل زاحف"
else return   "ولك ابوك يدري بيك "
end end end  end
return {
patterns = {
"^("..BOT_NAME.." اتفل)(.*)$", 
