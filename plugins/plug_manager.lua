...
local update = redis:get(basha..":VERSION")
...
sendDocument(msg.to.id, msg.id_,0, 1, nil, "./plugins/"..string.match (v, "(.*)%.lua") ..".lua", '🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ الباشا ¦ֆ \n📡¦ تابع قناة السورس @VPSBA\n👨🏽‍🔧', dl_cb, nil) end  else
if not plugin_exists(matches[2]) then 
return '🗂*¦* لا يوجد ملف بهذا الاسم .\n\n'
else sendDocument(msg.to.id, msg.id_,0,1,nil,"./plugins/"..matches[2]..".lua",'🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ الباشا ¦ֆ \n📡¦ تابع قناة السورس @VPSBA\n👨🏽‍🔧', dl_cb, nil)
end end end
